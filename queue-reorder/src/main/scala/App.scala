...
// A sort of generic test message for testing the various mechanisms...
...
object WidgetsGalore extends App {
  import WidgetViewModelActor._
  import system.dispatcher

  val system: ActorSystem = ActorSystem("helloAkka")

  val widgetViewModelActor: ActorRef = system.actorOf(
    WidgetViewModelActor.props(), 
    name="test-widget-viewmodel-actor")

  val cancellable = system.scheduler.schedule(
    200 milliseconds,
    500 milliseconds,
    widgetViewModelActor,
    WidgetMessage(1,1,1, WidgetCreated()) 
    )
}
